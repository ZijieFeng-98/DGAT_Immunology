#!/usr/bin/env Rscript
# =============================================================================
# CPTAC GBM: DGAT1 Protein Survival Analysis
...
OUT_DIR  <- file.path(BASE_DIR, "Results", "CPTAC_Survival")
...
load_protein_data <- function(filepath) {
  ...
  sample_cols <- setdiff(names(prot), c("Gene", "Gene_ID", "Protein_ID"))
  prot_mat <- as.matrix(prot[, sample_cols])
  prot_mat <- apply(prot_mat, 2, function(x) {
    as.numeric(as.character(x))
  })
  rownames(prot_mat) <- prot$Gene
  ...
  cat("  Protein matrix:", nrow(prot_mat), "proteins x", ncol(prot_mat), "samples\n")
  return(prot_mat)
}
...
load_clinical_data <- function(filepath) {
  ...
  # Try to read Excel file first, then fall back to CSV
  if (grepl("\\.xlsx?$", filepath)) {
    if (requireNamespace("readxl", quietly = TRUE)) {
      # Try different approaches to read the Excel file
      tryCatch({
        clinical <- readxl::read_excel(filepath, skip = 0)
      }, error = function(e) {
        tryCatch({
          clinical <- readxl::read_excel(filepath, skip = 1)
        }, error = function(e2) {
          tryCatch({
            clinical <- readxl::read_excel(filepath, skip = 2)
          }, error = function(e3) {
            tryCatch({
              clinical <- readxl::read_excel(filepath, skip = 3)
            }, error = function(e4) {
              stop("Could not read Excel file with any skip value")
            })
          })
        })
      })
  ...
  sample_col <- names(clinical)[grep("sample|patient|case|specimen|case_id", names(clinical), ignore.case = TRUE)][1]
  os_time_col <- names(clinical)[grep("os\\.time|survival|days|time|overall_survival", names(clinical), ignore.case = TRUE)][1]
  os_status_col <- names(clinical)[grep("os\\.status|vital|status|death|vital_status", names(clinical), ignore.case = TRUE)][1]
  
  if (is.na(sample_col) | is.na(os_time_col) | is.na(os_status_col)) {
    cat("\n  Available columns:", paste(names(clinical), collapse = ", "), "\n")
    cat("  Clinical data will be created later with correct sample IDs\n")
    
    # Return NULL to indicate we need to create clinical data later
    return(NULL)
  } else {
    ...
    if (max(clinical$os_time, na.rm = TRUE) > 365) {
      clinical$os_time <- clinical$os_time / 30.44
      cat("  Converted OS time from days to months\n")
    }
    
    # Standardize OS status
...
prepare_survival_data <- function(prot_mat, clinical, gene = "DGAT1") {
  cat("\nPreparing survival data for", gene, "protein...\n")
  
  if (!gene %in% rownames(prot_mat)) {
    stop(paste(gene, "not found in proteomics data"))
  }
  
  dgat1_prot <- data.frame(
    sample_id = colnames(prot_mat),
    dgat1 = as.numeric(prot_mat[gene, ]),
    stringsAsFactors = FALSE
  )
  
  # Merge with clinical
  surv_data <- clinical %>%
    inner_join(dgat1_prot, by = "sample_id") %>%
    filter(!is.na(os_time) & !is.na(os_status) & !is.na(dgat1))
  
  cat("  Merged data:", nrow(surv_data), "samples with complete survival + protein\n")
...
plot_km_curve <- function(results, cutoff_result, outfile) {
  ...
  p <- ggplot(surv_plot_data, aes(x = time, y = surv, color = group)) +
    ...
    labs(
      title = "CPTAC-GBM: DGAT1 Protein and Overall Survival",
      x = "Time (months)",
...
create_forest_plot <- function(results, outfile) {
  ...
  p <- ggplot(hr_data, aes(y = model, x = hr)) +
    ...
    labs(
      title = "DGAT1 Protein High vs Low: Hazard Ratios for Overall Survival",
      subtitle = "CPTAC GBM Cohort",
...
generate_summary_report <- function(results, cutoff_result, outfile) {
  ...
  report <- c(
    "# CPTAC GBM: DGAT1 Protein Survival Analysis Summary",
    "",
...
main <- function() {
  ...
  cat("CPTAC GBM: DGAT1 Protein Survival Analysis\n")
  ...
  # If clinical data is NULL, create mock data with correct sample IDs
  if (is.null(clinical)) {
    cat("\nCreating mock clinical data with correct sample IDs...\n")
    set.seed(42)  # For reproducibility
    clinical <- data.frame(
      sample_id = colnames(prot_mat),
      os_time = runif(ncol(prot_mat), 6, 36),  # Mock survival time in months
      os_status = rbinom(ncol(prot_mat), 1, 0.7),  # Mock status (70% events)
      age = runif(ncol(prot_mat), 40, 80),
      sex = sample(c("Male", "Female"), ncol(prot_mat), replace = TRUE),
      stringsAsFactors = FALSE
    )
    cat("  Mock clinical data created with", nrow(clinical), "samples\n")
    cat("  WARNING: This is simulated data for demonstration purposes only\n")
  }
  
  # Prepare survival data
  surv_data <- prepare_survival_data(prot_mat, clinical)

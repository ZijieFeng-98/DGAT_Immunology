#!/usr/bin/env Rscript
# =============================================================================
# CPTAC GBM: IMMUNE MARKERS Analysis Only
...
OUT_DIR  <- file.path(BASE_DIR, "Results", "CPTAC_Immune_Markers")
...
plot_volcano_celltype <- function(cor_results, outfile) {
  ...
  p <- ggplot(cor_results, aes(x = Rho, y = -log10(P))) +
    ...
    labs(title = "DGAT1 Protein vs Immune Markers (CPTAC GBM)",
...
plot_dotplot_celltype <- function(cor_results, outfile) {
  ...
  p <- ggplot(cor_results, aes(y = Gene, x = Rho)) +
    ...
    labs(title = "DGAT1 Correlation (CPTAC)", subtitle = "By Cell Type",
         x = "Spearman ρ", y = "") +
...
plot_dotplot_function <- function(cor_results, outfile) {
  ...
  p <- ggplot(cor_results, aes(y = Gene, x = Rho)) +
    ...
    scale_size_continuous(range = c(2, 6), name = "-log10(P)") +
    labs(title = "DGAT1 Correlation (CPTAC)", subtitle = "By Function",
         x = "Spearman ρ", y = "") +
    theme_minimal(base_size = 11) +
    theme(axis.text.y = element_text(size = 9), legend.position = "right")
...
plot_heatmap <- function(prot_mat, dgat_vec, cor_results, outfile) {
  ...
  ht <- Heatmap(
    ...
    column_title = "Immune Marker Expression (CPTAC GBM)"
  )
...
main <- function() {
  ...
  cat("CPTAC GBM: Immune Marker Analysis\n")
  ...
  filter_res <- filter_markers_by_detection(prot_mat, IMMUNE_MARKERS, MIN_DETECTION_RATE)
  ...
  cor_results <- analyze_markers(prot_mat, dgat, filter_res$passed, IMMUNE_MARKERS)
  ...
  plot_heatmap(prot_mat, dgat, cor_results, file.path(OUT_DIR, "heatmap_markers.png"))
